(function(m) {
  ...
  var encodeID = function(settings) {
    ...
    this.encode = function(inputData, useASCII) {
      var outputData = "";
      ...
      if (!(isNaN(inputData) ? !1 : (x = parseInt(inputData), (0 | x) === x))) {
        inputData = "s" + hexEncode(inputData, useASCII); // The "s" is to stop the leftTrim function removing the leading zeros required for unicode and only remove the padding zeros.
      }
    ...
    this.decode = function(inputData, useASCII) {
      ...
      if (!(isNaN(outputData) ? !1 : (x = parseInt(outputData), (0 | x) === x))) {
        outputData = hexDecode(outputData.substr(1), useASCII);
      } else {
        outputData = parseInt(outputData);
      }
    ...
    function hexEncode(inputString, useASCII) {
        var hexChar = null;
        var outputData = "";
        useASCII = (typeof(useASCII) == "undefined" ? false : useASCII);

        for (var i = 0; i < inputString.length; i++) {
          hexChar = inputString.charCodeAt(i).toString(16);
          if (useASCII) {
            outputData += ("0" + hexChar).slice(-2);
          } else {
    ...
    function hexDecode(inputHexedString, useASCII) {
      var outputString = "";
      ...
      if (useASCII) {
        hexSets = inputHexedString.match(/.{1,2}/g) || [];
      } else {
        hexSets = inputHexedString.match(/.{1,4}/g) || [];
      }
